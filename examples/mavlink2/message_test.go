...
func TestDecodeTwoMessages(t *testing.T) {
	...
	log.Printf("pktbytes len %d\n", len(pktbytes))

	decoder := &Decoder{
		Dialects: DialectSlice{DialectCommon},
	}

	msg1, tail, err := decoder.DecodeBytes(pktbytes)
	if err != nil || msg1 == nil  {
		t.Errorf("Decode fail: %s\n", err)
	} else {
		pktbytes = (*tail)[:]
		log.Printf("MSG 1. Tail len %d\n", len(pktbytes))
		log.Println(msg1.Bytes())
	}

	msg2, tail, err := decoder.DecodeBytes(pktbytes)
	if err != nil || msg2 == nil {
		t.Errorf("Decode fail: %s\n", err)
	} else {
		pktbytes = (*tail)[:]
		log.Printf("MSG 2. Tail len %d\n", len(pktbytes))
		log.Println(msg2.Bytes())
	}
