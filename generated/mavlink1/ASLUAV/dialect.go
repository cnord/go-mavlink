...
const (
	MAV_CMD_RESET_MPPT                         MAV_CMD = 40001 // Mission command to reset Maximum Power Point Tracker (MPPT). Params: 1) MPPT number; 2) Empty; 3) Empty; 4) Empty; 5) Empty; 6) Empty; 7) Empty;
	MAV_CMD_PAYLOAD_CONTROL                    MAV_CMD = 40002 // Mission command to perform a power cycle on payload. Params: 1) Complete power cycle; 2) VISensor power cycle; 3) Empty; 4) Empty; 5) Empty; 6) Empty; 7) Empty;
...
const (
	GSM_LINK_TYPE_NONE    GSM_LINK_TYPE = 0 // no service
	GSM_LINK_TYPE_UNKNOWN GSM_LINK_TYPE = 1 // link type unknown
	GSM_LINK_TYPE_2G      GSM_LINK_TYPE = 2 // 2G (GSM/GRPS/EDGE) link
	GSM_LINK_TYPE_3G      GSM_LINK_TYPE = 3 // 3G link (WCDMA/HSDPA/HSPA)
	GSM_LINK_TYPE_4G      GSM_LINK_TYPE = 4 // 4G link (LTE)
)
...
const (
	GSM_MODEM_TYPE_UNKNOWN      GSM_MODEM_TYPE = 0 // not specified
	GSM_MODEM_TYPE_HUAWEI_E3372 GSM_MODEM_TYPE = 1 // HUAWEI LTE USB Stick E3372
)
...
type CommandIntStamped struct {
	...
	Command          MAV_CMD   `gotype:"uint16"` // The scheduled action for the mission item, as defined by MAV_CMD enum
	TargetSystem     uint8     // System ID
	TargetComponent  uint8     // Component ID
	Frame            MAV_FRAME `gotype:"uint8"` // The coordinate system of the COMMAND, as defined by MAV_FRAME enum
...
type CommandLongStamped struct {
	...
	Command          MAV_CMD `gotype:"uint16"` // Command ID, as defined by MAV_CMD enum.
...
type GsmLinkStatus struct {
	...
	GsmModemType GSM_MODEM_TYPE `gotype:"uint8"` // GSM modem used
	GsmLinkType  GSM_LINK_TYPE  `gotype:"uint8"` // GSM link type
