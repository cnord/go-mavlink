...
const (
	// MAV_CMD_RESET_MPPT enum. Mission command to reset Maximum Power Point Tracker (MPPT). Params: 1) MPPT number; 2) Empty; 3) Empty; 4) Empty; 5) Empty; 6) Empty; 7) Empty;
	MAV_CMD_RESET_MPPT MAV_CMD = 40001
	// MAV_CMD_PAYLOAD_CONTROL enum. Mission command to perform a power cycle on payload. Params: 1) Complete power cycle; 2) VISensor power cycle; 3) Empty; 4) Empty; 5) Empty; 6) Empty; 7) Empty;
...
const (
	// GSM_LINK_TYPE_NONE enum. no service
	GSM_LINK_TYPE_NONE GSM_LINK_TYPE = 0
	// GSM_LINK_TYPE_UNKNOWN enum. link type unknown
	GSM_LINK_TYPE_UNKNOWN GSM_LINK_TYPE = 1
	// GSM_LINK_TYPE_2G enum. 2G (GSM/GRPS/EDGE) link
	GSM_LINK_TYPE_2G GSM_LINK_TYPE = 2
	// GSM_LINK_TYPE_3G enum. 3G link (WCDMA/HSDPA/HSPA)
	GSM_LINK_TYPE_3G GSM_LINK_TYPE = 3
	// GSM_LINK_TYPE_4G enum. 4G link (LTE)
...
const (
	// GSM_MODEM_TYPE_UNKNOWN enum. not specified
	GSM_MODEM_TYPE_UNKNOWN GSM_MODEM_TYPE = 0
	// GSM_MODEM_TYPE_HUAWEI_E3372 enum. HUAWEI LTE USB Stick E3372
